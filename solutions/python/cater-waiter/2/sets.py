...
def categorize_dish(dish_name, dish_ingredients):
    ...
    for index, category in enumerate(categories):
        if dish_ingredients.issubset(category):
            category_result = dish_name + ": " + category_names[index]
            break
...
def tag_special_ingredients(dish):
    ...
    for ingredient in dish[1]:
        if ingredient in SPECIAL_INGREDIENTS:
            special_ingreds.append(ingredient)
    return (dish[0], set(special_ingreds))
...
def singleton_ingredients(dishes, intersection):
    ...
    symmetric_difference_dishes = set()
    for dish in dishes:
        symmetric_difference_dishes ^= dish
    return symmetric_difference_dishes - intersection
